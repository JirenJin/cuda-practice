                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  int r = absolute_image_position_y;
  int c = absolute_image_position_x;
  float result = 0;
  for (int filter_r = -filterWidth/2; filter_r <= filterWidth/2; ++filter_r) {
    for (int filter_c = -filterWidth/2; filter_c <= filterWidth/2; ++filter_c) {
      ...
      // important to cast inputChannel value to float for intermediate computing
                      ...
                      unsigned char* const blueChannel)
{
  ...
  // get 1D index for accessing the global memory
  int idx = absolute_image_position_x + absolute_image_position_y * numCols;
                                ...
                                const float* const h_filter, const size_t filterWidth)
{
  ...
  checkCudaErrors(cudaMemcpy((void **) d_filter, (void **) h_filter, sizeof(float) * filterWidth * filterWidth, cudaMemcpyHostToDevice));
}
